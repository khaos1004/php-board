$( function()
{
	...
	$( document ).one( "click", "button[name='pwdch']", function()
	{
		...
		( {
			url: "../control/board/check_class_call.php",
			type: "POST",
			data: { "idx": $( "input[name='idx']" ).val(), "password": $( "input[name='password']" ).val(), "found": "passch" },
	...
	$( document ).on("click", "button[name='delete']", function()
	{
		if ( confirm( "삭제하시겠습니까?") )
		{
			$.ajax
			( {
				url: "../control/board/board_class_call.php",
				type: "POST",
				data: { "idx": $( "input[name='idx']" ).val(), "password": $( "input[name='password']" ).val(), "found": "delete" } ,
				success : function( ch )
				{
					if ( ch == "true" )
					{
		...
		if ( $( "input[name='title']" ).val() != "" && $( "input[name='writer']" ).val() != "" && $( "textarea[name='contents']" ).val() != "" )
		{
			if ( confirm( "수정하시겠습니까?" ) )
			{
				...
				( {
					url: "../control/board/board_class_call.php",
					type: "POST",
					data: { "idx": $("input[name='idx']").val(), "title": $( "input[name='title']" ).val(), "writer": $( "input[name='writer']" ).val(), "contents": $( "textarea[name='contents']" ).val(), "found": "update" },
					success : function( suc )
					{
						if ( suc == "true" )
						{
	...
	$( "button[name='re']" ).on( "click", function()
	{
		if ( /^[0-9]{4}$/.test ( $( "input[name='replypwd']" ).val() ) )
		{
			$.ajax
			( {
				url: "../control/board/reply_class_call.php",
				type: "POST",
				data: { "board_idx": $( "input[name='idx']" ).val(), "name": $( "input[name='replyid']" ).val(), "password": $( "input[name='replypwd']" ).val(), "contents": $( "textarea[name='replycontents']" ).val(), "found": "create" },
				success : function( ch )
				{
					console.log(ch);
					if ( ch == "true" )
	...
	$( document).on( "click", "button[name='replypwdch']", function()
	{
		...
		( {
			url: "../control/board/reply_class_call.php",
			type: "POST",
			data: { "idx": now_button.parents("div").attr("idx"), "password": now_button.prev("input[name='reply_editch']").val(), "found": "passwordch" },
			success : function( ch )
			{
				if ( ch == "true" )
				{
	...
	$( document).on( "click", "button[name='editdone']", function()
	{
		...
		( {
			url: "../control/board/reply_class_call.php",
			type: "POST",
			data: { "idx":now_button.parents("div").attr("idx"), "name": now_button.prevAll("input[name='replyid_read']").val(), "contents": now_button.parents("div").find("textarea[name='replycontents_read']").val(), "found": "update" },
			success : function( ch )
			{
				if ( ch == "true" )
				{
	...
	$( document).on( "click", "button[name='replydel']", function()
	{
		...
		( {
			url: "../control/board/reply_class_call.php",
			type: "POST",
			data: { "idx":now_button.parents("div").attr("idx"), "password": now_button.prevAll("input[name='reply_editch']").val(), "found": "delete" },
			success : function( ch )
			{
				if ( ch == "true" )
				{
